...
export const PredictiveDistributionChart: React.FC<Props> = ({
  ...
  if (!isSelectedActionPreregistered || !isValid) {
    return (
      <div className="w-full flex flex-col p-8 bg-[#FCFCFA] rounded-xl border border-[#D9DFDB] text-center items-center justify-center min-h-[360px]">
        <div className="w-10 h-10 rounded-full bg-[#FEF2F2] border border-[#FECACA] flex items-center justify-center mb-3">
          <Info className="w-5 h-5 text-[#DC2626]" />
        </div>
        <h4 className="text-sm font-bold text-[#17201F] mb-1">
          Predictive Distribution Unavailable in Snapshot
        </h4>
        <p className="text-xs text-[#66706C] max-w-lg mb-4 leading-relaxed">
          In the authentic scientific evidence ledger, serialized predictive distributions <span className="font-mono text-3xs bg-[#F4F3EE] px-1.5 py-0.5 rounded">p(y | a, Hₖ)</span> are stored exclusively for the selected &amp; preregistered winner action (<strong className="text-[#DC2626]">{preregAction?.candidate_id} {preregAction?.action_type}</strong>). Alternative candidate counterfactuals evaluated at this step do not reuse winner curves.
        </p>
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#F4F3EE] border border-[#D9DFDB] rounded-lg text-2xs font-mono text-[#17201F]">
          <span>Inspected Alternative:</span>
  ...
  return (
    <div className="w-full flex flex-col">
      {/* Header with Title and Observable Dropdown */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 mb-3">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <label className="text-2xs font-mono text-[#8F9995] whitespace-nowrap">Observable:</label>
          <select
            value={selectedObsIndex}
            onChange={(e) => setSelectedObsIndex(Number(e.target.value))}
            className="text-2xs font-mono bg-white border border-[#D9DFDB] rounded-lg px-2.5 py-1 text-[#17201F] shadow-2xs cursor-pointer focus:ring-1 focus:ring-[#DC2626]"
          >
      ...
      <div className="mb-3">
        {isBlinded ? (
          <div className="flex items-center justify-between px-3 py-1.5 bg-[#FEF3C7] border border-[#FDE68A] rounded-lg text-2xs text-[#92400E] gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              <Lock className="w-3.5 h-3.5 text-[#D97706] shrink-0" />
              <span className="font-bold whitespace-nowrap">Observation Blinding Firewalled:</span>
              <span className="truncate">
                Ground-truth measurement strictly hidden prior to preregistration lock.
              </span>
            </div>
            <span className="text-3xs font-mono uppercase tracking-wider font-bold bg-[#FDE68A] px-2 py-0.5 rounded text-[#92400E] whitespace-nowrap shrink-0">
              State {revealPhase === 'A_SCORED' ? 'A (Scored)' : 'B (Locked)'}
            </span>
          </div>
        ) : (
          <div className="flex items-center justify-between px-3 py-1.5 bg-[#FEF2F2] border border-[#FECACA] rounded-lg text-2xs text-[#991B1B] gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              <Eye className="w-3.5 h-3.5 text-[#DC2626] shrink-0" />
              <span className="font-bold whitespace-nowrap">Evidence Revealed:</span>
              <span className="truncate">
                Observed value: <strong className="font-mono">{observedValue?.toFixed(4) ?? 'N/A'}</strong>. Log Bayes factors applied to belief update.
              </span>
            </div>
            <span className="text-3xs font-mono uppercase tracking-wider font-bold bg-[#FECACA]/40 px-2 py-0.5 rounded text-[#991B1B] whitespace-nowrap shrink-0">
      ...
      {/* Guaranteed Height Chart Container */}
      <div className="w-full h-[300px]">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={chartData} margin={{ top: 12, right: 30, left: 0, bottom: 24 }}>
      ...
      <div className="mt-3 pt-2.5 border-t border-[#D9DFDB] flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-2xs font-mono text-[#66706C]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-bold text-[#17201F] whitespace-nowrap">
            {selectedModality} on {selectedCandidateId}
          </span>
        </div>
        <div className="flex items-center gap-1 text-[#DC2626] ml-2">
          <ShieldCheck className="w-3.5 h-3.5 text-[#DC2626] shrink-0" />
          <span className="whitespace-nowrap font-medium">Strict Pre-reveal Preregistration Invariant Verified</span>
        </div>
      </div>
    </div>
  );
